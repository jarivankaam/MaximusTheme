...
$display_mode        = get_sub_field('gallery_display_mode'); // 'preview' of 'full'
$read_more_label     = get_sub_field('read_more_label') ?: __('Read More', 'framework');
$show_less_label     = __('Show Less', 'framework');
?>

<section id="<?= esc_attr( $section_id ) ?>" class="section-gallery <?= $display_mode === 'preview' ? 'mode-preview' : 'mode-full' ?>">
    <div class="container">
        ...
        <?php if ( $total_images > 0 ) : ?>
            ...
            <?php if ( $display_mode === 'preview' && $total_images > 3 ) : ?>
                <button class="read-more-btn"><?= esc_html( $read_more_label ) ?></button>
            <?php endif; ?>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        document.querySelectorAll('.section-gallery.mode-preview').forEach(function(section) {
            ...
            function calcPreviewHeight() {
                const gap = parseInt(getComputedStyle(gallery).rowGap) || 0;
                let h = 0;
                items.forEach((item, i) => {
                    const itemH = item.getBoundingClientRect().height;
                    if (i < previewQty) {
                        h += itemH + gap;
                    } else if (i === previewQty) {
                        h += (itemH + gap) * 0.3;
                    }
                });
                return h;
